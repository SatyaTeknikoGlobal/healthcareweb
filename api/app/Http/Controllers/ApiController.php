...
// }
    ...
    public function notification_list(Request $request){
        ...
        $notifications = DB::table('notifications')->select('id','user_id','text','image','description')->where('user_id',$user->id)->get();
    ...
    public function emergency(Request $request){
        ...
        if(!empty($guards)){
            foreach($guards as $guard){
                ...
                $dbArray1['title'] = 'Emergency From User';
                ...
                if(!empty($logins)){
                    foreach($logins as $login){
                        //$deviceToken = $login->deviceToken;
                        $title = 'Message From Users';
                        $body = ['id'=>(string)$user->id,'name'=>$user->name,'phone'=>$user->phone];
                        $deviceToken = 'diHRJwgoQk6Pv17Q-xizV-:APA91bE9M_Oodk194oVHepycFltQg9Ebo7_Srudlnj_sH5eXGP3bgsaGWQ_YoHA0t27fWGlTGgcg1bzELLWg8nP_2xPf_YQtYNLYcK8c7Xjwd2s-CL2qeJwDr5_RDQ6qwVC9oPzsSob_';
                        $type = 'emergency';
                        $success = $this->send_notification($title, $body, $deviceToken,$type);
